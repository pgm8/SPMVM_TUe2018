...
"The goal of this thesis is to provide a comprehensive and self-contained analysis of a class of algorithms known as" nearest neighbors and random forests \citep{ref:Breiman2001} and their aptness as estimators of time-varying correlation between financial returns.  \\
...
\subsubsection{Diagnosing the Generalization Error of KNN}
...
\subsubsection{Alternative KNN Parameterizations}
...
\noindent
\textbf{Insert figures of correlation dynamics and state that it resembles assuming constant correlation, which is the exact concept we are trying to improve on with time-varying correlations.}
...
\subsubsection{Diagnosing the Generalization Error of RF}
...
\subsubsection{Alternative RF Parameterizations}
...
\begin{enumerate}
	...
	\item KNN with true correlation outperforms moving window estimates particularly for smaller window sizes but with full training set and potentially inverse distance weighting function for all window sizes in the interval [3, 100]. The latter results however in more or less constant correlation, the very idea we try to improve on by considering correlation to be time-varying. Optimization over hyperparameter $k$ shows that with an increase in $k$ variance is reduced (and bias initially) and correlation estimates become increasingly smooth.
