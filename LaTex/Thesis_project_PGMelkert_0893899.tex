...
\noindent
Table \ref{tab:stat_dist_gjr_sstd_table} summarizes the results of estimating the GJR-GARCH(1,1) model from \ref{eq:gjr_garch} on the 30 time series. The average estimated degrees of freedom of the shape parameter is 6.9 and the estimated skewness parameter is positive for all 30 assets, and both estimated parameters are significantly different from zero for all 30 assets, indicating heavy tails and positive skewness.  
...
\multicolumn{6}{l}{LM test for squared standardized residuals} & \multicolumn{2}{c}{2}                  \\
...
\noindent
\textbf{The GARCH model can capture part of the heavy tail, capture observed leptokurtic behavior. Therefore, potentially, all excess kurtosis is captured by GARCH with Gaussian assumptions for marginal error distributions. Potentially, a more heavy tailed distribution is needed. Check whether estimated skew and kurtosis coefficients are significantly different from zero.
}
