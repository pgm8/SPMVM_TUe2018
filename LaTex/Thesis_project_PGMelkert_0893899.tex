...
\vfill
\noindent
Subject headings: Dynamic Conditional Correlation; Semiparametric Multivariate GARCH; Nearest Neighbor; Random Forest; time-varying covariances.  
...
\noindent
An alternative to the parametric estimation of the conditional covariance matrix and conditional correlation matrix is provided by non- and semiparametric models. In contrast to parametric estimation, nonparametric models do not impose (potentially misspecified) arbitrary assumptions on the distribution or functional form of the object being estimated. Semiparametric models comprise methods that contain both fully parametric components and fully nonparametric components, and may prove to be appropriate in cases nonparametric models are not;  for example, in case of implications of the curse of dimensionality or lack of knowledge on the distribution of errors \citep{ref:Li2006}. Although semiparametric models may inherit desirable properties of a nonparametric model in that they are robust against misspecification on distributional assumption and functional form, the reliance on parametric assumptions does not exclude semiparametric models from inconsistency due to misspecification. Although there are numerous papers on nonparametric models for the conditional variance, there is a limited number of papers on nonparametric multivariate volatility models\footnote{There are a number of papers that consider nonparametric estimation of the conditional mean and/ or conditional volatility in univariate settings. This is not the focus of our paper though, therefore these are not mentioned.}. Existing semiparametric multivariate volatility approaches may be based on the premise of a finite-dimensional parameter functional form for the conditional covariance matrix while the error distribution is estimated nonparametrically, or on the premise of a finite-dimensional parameter functional form for the conditional variance while the conditional correlations are estimated nonparametrically. For some interesting semiparametric (time-varying) copula-based multivariate GARCH models belonging to the first category\footnote{This class of models considers the conditional variance of each marginal time series to be specified parametrically through univariate GARCH-type models where non? or semi?parametric estimation of the univariate marginal distributions is followed by a parametric estimation of the copula function parameters, which specifies the dependence structure between the marginal series.\iffalse the multivariate distribution of the standardized innovation are specified semiparametrically as a parametric copula evaluated at nonparametric marginals.\fi}, examine the work of  \cite{ref:Chen2006}, \cite{ref:Patton2006}, \cite{ref:Jondeau2006}, \cite{ref:Ausin2010}, and \textbf{Add some more recent ones}. For some applications where the error distribution is estimated nonparametrically using kernel density estimation, examine the work of \cite{ref:Hafner2007}\footnote{An infinite-dimensional parameter functional form is considered for the error distribution, which is estimated nonparametrically using kernel density estimation.},  
 
\bigskip
\noindent
For some interesting applications belonging to the second category, examine the work of \cite{ref:Hafner2006}\footnote{A nonparametric estimator is considered for the correlation function where a Nadaraya-Watson kernel regression is applied to each conditional correlation individually.}     



\bigskip
\noindent
Make a distinction in semi-parametric models between: \\
- nonparametrically multivariate error distribution:  , \cite{ref:Hafner2007}  \\
	+ copula based  \cite{ref:Chen2006} \\
	+ kernel density estimation \\
- semi/non parametrically conditional correlations:  \cite{ref:Hafner2006}


\bigskip

\noindent
For some interesting applications, see the work of \cite{ref:Long2005}, \cite{ref:Hafner2006}\footnote{Instead of a priori imposing a certain parametric specification for the correlation functions, they suggest a nonparametric estimator where a Nadaraya-Watson kernel regression is applied to each conditional correlation individually.}, \cite{ref:Hafner2007}\footnote{A semiparametric multivariate volatility model is proposed that considers a finite-dimensional parameter functional form for the conditional covariance matrix and an infinite-dimensional parameter functional form for the error distribution. Distribution of errors is considered nonparametrically using kernel density estimation.}, \cite{ref:Chen2006}\footnote{}, and \cite{ref:Fox2011}.   \\

\noindent
The aim of this paper is to develop a new semiparametric model for the conditional covariance matrix that is flexible and remains easy to estimate in high dimensional systems. The model is semiparametric in that it combines parametric univariate Generalized Auto Regressive Conditional Heteroskedasticity specifications for the individual conditional volatilities with nonparametric machine learning regressors for the conditional correlations. In the past decade, machine learning has been responsible for significant advances in self-driving cars, speech and facial recognition, effective web search, and in a vast amount of medical diagnostic procedures. However, the approach of using machine learning for the estimation of conditional correlation between financial variables has not been extensively investigated in scientific literature. In fact, there is a limited number of papers on this type of machine learning applications; for initial applications, see the papers of \cite{ref:Basturk2016_1} and \cite{ref:Basturk2016}.  \\
...
\noindent
In this paper, machine learning estimators are proposed for modeling time-varying correlation between financial returns. Two nonparametric learning algorithms, nearest neighbor and random forest, are used to point estimate  conditional correlations directly and independently of conditional volatilities. These learning methods have a clear advantage over multivariate GARCH models in that arbitrary (potentially misspecified) assumptions on the distribution and functional form of the conditional correlation matrix are avoided. \textbf{Any computational advantages that shows applicability to large correlation matrices? What about curse of dimensionality and application of new models to study the volatility and correlation of large-size portfolios, potentially comprising thousands of assets, which is of interest to banks and hedge funds.}  \\
...
\noindent
In an empirical application, the proposed semiparametric multivariate volatility model is applied to the 30 constituents included in the Dow Jones Industrial Average index. Performance is verified in terms of an economic loss function, the Value-at-Risk, as the true conditional covariance matrix is unobservable. It is demonstrated that our semiparametric model is able to capture interesting time-varying properties in correlations and that it is \textit{competitive with a parametric counterpart, the Dynamic Conditional Correlation model of \cite{ref:Engle2002}}.  \\




\noindent
The remained of the paper is organized as follows. Chapter 2 gives an overview of the methodological underpinnings of multivariate volatility modeling and machine learning. Chapter 3 describes a simulation experiment and results are presented. Section 4 presents results of an empirical application to the 30 constituents included in the Dow Jones Industrial Average index. Finally, conclusions and directions for future research are provided in Chapter 5. \\
...
\begin{enumerate}
	...
	\item Given the supervised learning problem formulation, design and implement a Nearest Neighbor and Random Forest estimator with support for (multivariate) regression output problems.  
...
The main contribution of this paper is the development of a computational framework using nearest neighbor and random forest algorithms for the task of estimating time-varying correlation between financial returns. Moreover, we show that the integration of statistical models for capturing conditional volatility, such as univariate GARCH, with proposed learning estimators provides a flexible, semiparametric way of formulating multivariate volatility models that avoid strong distributional assumptions nor impose any (misspecified) structure on the correlation process.  \\
...
In this chapter we will develop some notation and basic approaches that will provide us with the foundational concepts used throughout this paper. As such, Section \ref{sec:ml_supervised} introduces the concepts of machine learning and supervised learning. Theoretical background is given of the learning algorithms that lie at the core of k-nearest neighbor and random forest as well as the effect of foundational model properties on the generalization error. The latter is done through discussion of an important issue in machine learning, i.e. the tradeoff between bias and variance. Next, Section \ref{sec:mvm} introduces the conceptual underpinnings of multivariate volatility modeling and the decomposition of the variance-covariance matrix that is used in this paper to identify conditional variances and correlation. Finally, Section \ref{sec:cor_ml} brings the topics of the preceding two sections together; we will discuss what approach to take to successfully address supervised learning in the context of estimating time-varying correlation between financial returns.
...
\noindent
We examine the DCC model further in detail in the next section as the DCC model will be compared with our semiparametric multivariate volatility model in Chapter \ref{chap:multivariate_analysis}.
...
\noindent
"We estimate the VaR via the Monte Carlo method, which implies full repricing of the portfolio at every point in time. To keep a balance between the computational costs and the accuracy of the results we simulate five thousand portfolio values for each point in time." \\

\noindent
On the basis of the fitted models, we generate B = 1000 bootstrap samples, $\{Y^b_{t+1} = Y^b_{t+1,1}, Y^b_{t+1,2}, \dots,Y^b_{t+1,k}\}^B_{b=1}$ for day t+1 and estimate the conditional VaR by using the B bootstrapped portfolio returns, $\{\Sigma_{i=1}^k w_i Y^b_{t+1,i}\}^B_{b=1}$. A violation happens when the actual daily loss is over the qth quantile of thebootstrap sample, where q varies among 0.95, 0.99 and 0.995. \\
...
"the assumption  of multivariate normal distribution on et is not consistent with the characteristics of financial
data, such as fat tail and asymmetric dependence. " \\
