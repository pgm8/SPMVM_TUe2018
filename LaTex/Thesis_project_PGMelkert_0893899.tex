...
\noindent
The computational framework development using nearest neighbor and random forest for estimating time-varying correlation was motivated by 1) the simplicity of knn 2) performance of random forest on a variety of supervised learning problems. \\
...
our modeling framepaper can lead to accuracy improvements in forecasting, especially those for
...
\begin{enumerate}
	...
	\item Move on to topic of random forest: proposed by breiman (2001). Combination of decision tree, bagging and random feature subspacing.
...
the two trees is the application of random feature subspacing in RF-CART trees ? whereas C4.5 examines all possible features to find the best split in any given node, the trees in Random Forest examine only a random subsample of features in each node.} \\
...
\subsubsection{Random Feature Subspacing}
...
that maximizes impurity decrease. I think in the setting of multiple outputs scikit learn decision tree compute the average impurity decrease over the q output variables (but I am not sure, cant really confirm from reading the code on github). \\
...
Aside from being simple to use, the method is generally recognized for its accuracy and its ability to deal with small sample sizes and high-dimensional feature spaces. At the same time, it is easily parallelizable and has therefore the potential to deal with large real-life systems....  J. Howard (Kaggle) and M. Bowles (Biomatica) claim in Howard and Bowles (2012) that ensembles of decision trees?often known as ?random forests??have been the most successful general-purpose algorithm in modern times, while H. Varian, Chief Economist at Google, advocates in Varian (2014) the use of random forests in econometrics... \\
\fi
...
\noindent
One should note that the set of covariates and response variable in the dataset of \eqref{eq:dataset} both contain noise in the form of approximation error as both are approximations of correlation and not the true correlation. A consequence of noisy data is the risk of overfitting and this is something to take into consideration during the simulation study in Chapter \ref{ch:simulation_study}.   
...
\section{KNN, RF Application with True Correlations for Response Variable and Proxies for Covariates} \label{sec:true_correlation}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{KNN, RF Application with Proxies for Response Variable and Covariates} \label{sec:proxy_correlation}
...
\noindent
As in Section \ref{sec:knn_true_cor}, two more parameterizations of the KNN learning algorithm are analysed where the set of neighbors used for point estimation of time-varying correlation is defined as the entire training set. The KNN algorithm uses an uniformly weighted average of correlations and an inverse distance weighted average of correlations, as defined in \eqref{eq:IDWfunction} \citep{ref:Shepard1968} in the first and second parameterizations, respectively. The MSE decompositions of both parameterizations with proxies for covariates and the response variable is depicted in Fig. \ref{fig:decom_mse_knn_len_train_IDW_pearson_kendall_proxy}. The variance term becomes negligible small in case the set of neighbors used for point estimation of time-varying correlation is defined as the entire training set, regardless of the weighting function. The red line of the variance is barely observable in the bottom of Fig. \ref{fig:decom_mse_knn_len_train_pearson_proxy}-\ref{fig:decom_mse_knn_IDW_kendall_proxy}. This is in line with results presented in Tabel \ref{tab:mse_decomp_knn_pearson_kendall_proxy} where the value of the variance term is negligible small for larger $k$, regardless whether the set of covariates is constructed from Pearson or Kendall moving window estimates of correlation.
...
This part of the analysis concerns modeling the dependence structure in a multivariate setting. We continue to opt for correlation as a measure for dependency. However, the focus of analysis in a multivariate setting is a \textit{system of correlations} rather than \textit{individual correlations}.  \\



\noindent
\textbf{Say we will look at VaR forecasting, relate Basel to this, and say we take 30 DJIA constituents as asset universe. Then add a Figure with the sample period 1987-2001 of the log returns of the DJIA index. Then the setting is set.} \\
...
The dynamic conditional correlation (DCC) model is a class of models that generalizes univariate generalized autoregressive conditional heteroskedasticity (GARCH) models to the multivariate case. The DCC models capture the advantages of GARCH models and simplifies the computation of multivariate GARCH models such as VEC-GARCH and BEKK. Reparametrization of covariance matrices lets the DCC model take advantage of the fact that correlation matrices are generally easier to model than the direct modeling of the covariance matrices. In the DCC models, time series of daily mean-corrected log returns on k assets $y_t=a_t$ is given by
