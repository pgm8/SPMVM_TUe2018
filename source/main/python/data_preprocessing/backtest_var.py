...
class Backtest:
    ...
    def compute_var(alpha, cov_mat, weight_vec):
        variance = np.dot(np.dot(weight_vec, cov_mat), weight_vec)
        std_dev = np.sqrt(variance)
        return stats.norm.ppf(alpha) * std_dev
...
if __name__ == "__main__":
    ...
    T_tranquil, T_volatile = 504, 500  # out-of-sample length market period
    n = 30  # Number of assets under consideration
    w = [1 / n] * n
    VaR_true_tranquil = data_DJIA_tranquil.tail(T_tranquil).dot(w)
    VaR_true_volatile = data_DJIA.tail(T_volatile).dot(w)

    var_forecast = mm.load_data('multivariate_analysis/VaR/tranquil/var_RF10_kendall_gjr_1994_1995.pkl')

    bt = Backtest(actual=VaR_true_tranquil, forecast=var_forecast.loc[:, "0.99"], alpha=0.01)
    bt.plot()
